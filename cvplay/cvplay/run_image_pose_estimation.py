...
def image_pose_estimation(id, location, model_name):
    ...
    image = common.read_imgfile(location, None, None)
    if image is None:
        logger.error('Image can not be read, path=%s' % location)
        sys.exit(-1)
    ...
    logger.info('inference image: %s in %.4f seconds.' % (location, elapsed))
